...
import { fetchPropertyById, updateProperty, PropertyUpdate } from '../store/slices/propertySlice';
...
const PropertyEditPage: React.FC = () => {
  ...
  const [property, setProperty] = useState<PropertyUpdate>({});
  ...
  useEffect(() => {
    if (currentProperty && !isLoading) {
      setProperty({
        ...
        securityDeposit: currentProperty.securityDeposit,
        isAvailable: currentProperty.isAvailable,
      });
      
      setSelectedAmenities(currentProperty.amenities || []);
    }
  ...
  const validateForm = () => {
    ...
    if (property.bedrooms && property.bedrooms < 1) newErrors.bedrooms = 'Must have at least 1 bedroom';
    if (property.bathrooms && property.bathrooms < 1) newErrors.bathrooms = 'Must have at least 1 bathroom';
    if (property.monthlyRent && property.monthlyRent <= 0) newErrors.monthlyRent = 'Monthly rent must be greater than 0';
    if (property.securityDeposit && property.securityDeposit < 0) newErrors.securityDeposit = 'Security deposit cannot be negative';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm() && id) {
      dispatch(updateProperty({ id: parseInt(id), property: updatedProperty }))
        .unwrap()
        .then(() => {
  ...
  if (!isOwner) {
    return (
      <Paper sx={{ p: 5, textAlign: 'center' }}>
        <Typography variant="h6">You don't have permission to edit this property</Typography>
        <Button
          variant="contained"
          color="primary"
          sx={{ mt: 2 }}
